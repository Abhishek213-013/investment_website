  ...
  <InvestProLayout>
    <div class="min-h-screen bg-gray-50">
      ...
      <main class="container mx-auto px-6 py-8">
        ...
        <div v-else-if="!pageData || !processedSections || processedSections.length === 0" class="text-center py-12">
        ...
        <div v-else class="space-y-8">
          <div v-for="(section, index) in processedSections" :key="section.page_section_id || index" 
            ...
            <div :class="getSectionLayoutClasses(section)">
              ...
              <div v-if="sectionHasContent(section)" :class="getContentClasses(section)">
                <div class="prose max-w-none" :class="getContentAlignmentClass(section)">
                  ...
                  <div v-else class="text-gray-500 italic text-center py-8">
                    {{ currentLanguage === 'en' ? 'No content available for this section' : 'এই বিভাগের জন্য কোনো কন্টেন্ট উপলব্ধ নেই' }}
                  </div>
                </div>
              </div>
              ...
              <!-- No Content or Attachments Message -->
              <div v-if="!sectionHasContent(section) && !sectionHasAttachments(section)" 
                   class="text-center py-8 text-gray-500 italic">
                {{ currentLanguage === 'en' ? 'This section has no content or attachments' : 'এই বিভাগে কোনো কন্টেন্ট বা সংযুক্তি নেই' }}
              </div>
            </div>
          </div>
        </div>
...
const processedSections = computed(() => {
  ...
  return pageData.value.sections.map((section, index) => ({
    page_section_id: section.page_section_id || `section-${index}-${Date.now()}`,
