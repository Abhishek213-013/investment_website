  ...
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="flex justify-between items-center">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">
              {{ currentLanguage === 'en' ? page.page_name : (page.page_name_bn || page.page_name) }}
            </h1>
          </div>
    ...
    <main class="container mx-auto px-6 py-8">
      ...
      <div v-else class="space-y-8">
        <div v-for="(section, index) in page.sections" :key="section.page_section_id" 
          ...
          <div :class="getSectionLayoutClasses(section)">
            ...
            <!-- Content Area -->
            <div v-if="section.hasContent" :class="getContentClasses(section)">
            ...
            <!-- Attachments Area -->
            <div v-if="section.hasAttachments" :class="getAttachmentClasses(section)">
...
const currentLanguage = ref('en') // 'en' or 'bn'
...
const getSectionHeading = (section) => {
  return currentLanguage.value === 'en' ? section.heading : (section.heading_bn || section.heading)
}

const getSectionDescription = (section) => {
  return currentLanguage.value === 'en' ? section.description : (section.description_bn || section.description)
}

const getSectionContent = (section) => {
  return currentLanguage.value === 'en' ? section.content : (section.content_bn || section.content)
}
...
const getHeaderAlignmentClass = (section) => {
  ...
  return 'text-left max-w-4xl' // Default for 'L'
}
...
const getContentAlignmentClass = (section) => {
  ...
  return 'text-left' // Default for 'L'
}
...
const getSectionLayoutClasses = (section) => {
  ...
  if (section.hasContent && section.hasAttachments) {
    ...
    // For single column, apply alignment to the entire section
    if (section.content_allignment === 'C') {
...
const getContentClasses = (section) => {
  ...
  if (section.hasAttachments) {
    ...
    // For single column with center alignment
    if (section.content_allignment === 'C') {
...
const getAttachmentClasses = (section) => {
  ...
  if (section.hasContent) {
    ...
    // For single column attachments with alignment
    if (section.content_allignment === 'C') {
      classes.push('text-center', 'mx-auto')
    } else if (section.content_allignment === 'R') {
      classes.push('text-right', 'ml-auto')
...
const getImageAttachments = (attachments) => {
  return attachments.filter(att => isImage(att))
}
...
const nextSlide = (sectionId) => {
  ...
  const images = getImageAttachments(page.value.sections.find(s => s.page_section_id === sectionId)?.attachments || [])
...
onMounted(() => {
  // Ensure sections have the correct flags for display
  if (page.value.sections) {
    page.value.sections = page.value.sections.map(section => ({
      ...section,
  ...
  // Initialize slide states and auto-slide
